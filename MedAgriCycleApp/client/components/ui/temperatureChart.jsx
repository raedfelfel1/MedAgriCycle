...
export default function TemperatureChart({ sensors, hours }) {
 ...
 useEffect(() => {
    ...
    if (hours === "live") {
      ...
      const tick = async () => {
        try {
          ...
          const row = raw[raw.length - 1]; // ⬅️ récupérer le vrai dernier point
  ...
  const chartData = {
    ...
    legend: [hours === "live" ? "Température (Live)" : t("temperature") + " (°C)"],
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>
        {hours === "live" ? "Température - LIVE" : ""+t("temperature")+" ("+hours+"h)"}
      </Text>
